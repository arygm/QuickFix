...
class SearchWorkerResultScreenTest {
    ...
    fun testBannerImageIsDisplayed() {
        ...
        composeTestRule.onNodeWithTag("sliding_window_banner_image").assertExists()
            .assertIsDisplayed()
    }
    ...
    fun testTagsAreDisplayed() {
        ...
        composeTestRule.onNodeWithTag("sliding_window_tags_flow_row").assertExists()
            .assertIsDisplayed()
        ...
        tags.forEach { tag ->
            composeTestRule.onNodeWithText(tag).assertExists().assertIsDisplayed()
        }
    }
    ...
    fun testWorkerSortingByRating() {
        ...
        sortedWorkers.forEachIndexed { index, worker ->
            workerNodes[index].assert(
                hasText("${worker.price.roundToInt()}", substring = true)
            )
        }
    }
    ...
    fun testNoMatchingWorkers() {
        ...
        composeTestRule.onNodeWithText("Service Type").performClick()
        composeTestRule.waitForIdle()
        composeTestRule.onNodeWithText("Carpenter")
            .performClick() // No workers with "Carpenter" tag
        composeTestRule.onNodeWithText("Apply").performClick()
        composeTestRule.waitForIdle()
    ...
    fun testTogglingRatingFilterOff() {
        ...
        workerNodes[0].assert(hasText("4.5 ★", substring = true))
        workerNodes[1].assert(hasText("3.0 ★", substring = true))
        workerNodes[2].assert(hasText("2.0 ★", substring = true))
        ...
        val workerNodesAfterRevert =
            composeTestRule.onNodeWithTag("worker_profiles_list").onChildren()
        workerNodesAfterRevert[0].assert(hasText("3.0 ★", substring = true)) // w1 first again
        workerNodesAfterRevert[1].assert(hasText("4.5 ★", substring = true))
        workerNodesAfterRevert[2].assert(hasText("2.0 ★", substring = true))
    }
    ...
    fun testTogglingFilterButtonsVisibility() {
        ...
        // Initially, the filter_buttons_row might not be visible until we click the tune button
        composeTestRule.onNodeWithTag("filter_buttons_row").assertDoesNotExist()
