...
fun QuickFixPriceRange(
    ...
    Canvas(
                ...
                onTouchEvent = { motionEvent ->
                    when (motionEvent.action) {
                        MotionEvent.ACTION_DOWN -> {
                            val x = motionEvent.x
                            val y = motionEvent.y
                            val dis1 = sqrt((x - leftCircleOffset.x).pow(2) + (y - leftCircleOffset.y).pow(2))
                            val dis2 = sqrt((x - rightCircleOffset.x).pow(2) + (y - rightCircleOffset.y).pow(2))

                            if (dis1 < circleRadiusInPx) { // left circle clicked
                                leftCircleDragging = true
                            } else if (isDoubleSlider && dis2 < circleRadiusInPx) { // right circle clicked (only if double)
        ...
        val topL = leftCircleOffset.y - tooltipSpacing.toPx() - circleRadiusInPx - tooltipHeight.toPx()

        val leftR = if (isDoubleSlider) rightCircleOffset.x - tooltipWidth.toPx() / 2f else 0f
        val topR = if (isDoubleSlider) rightCircleOffset.y - tooltipSpacing.toPx() - circleRadiusInPx - tooltipHeight.toPx() else 0f
